 ...
 *
 * Les messages sont éclatés en un fichier par domaine sous `messages/<locale>/`, puis
 * fusionnés ici en un seul objet dont chaque nom de fichier devient l'espace de noms.
 * Un catalogue unique de plusieurs milliers de clés devient illisible, et deux
 * personnes traduisant deux écrans différents finiraient par se marcher dessus.
 */
 ...
 */
const ESPACES = ['commun', 'vitrine'] as const;
  ...
  const entrees = await Promise.all(
    ESPACES.map(async (espace) => {
      try {
        ...
        // l'écran, alors qu'une erreur 500 rend la vitrine inaccessible.
...
export default getRequestConfig(async () => {
  ...
  return {
    ...
    /** Instant de référence des formats relatifs, partagé serveur et client. */
    now: new Date(),
  };
});
